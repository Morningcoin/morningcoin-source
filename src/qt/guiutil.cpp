...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("morningcoin"))
        return false;
...
{
    // Convert morningcoin:// to morningcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("morningcoin://"))
    {
        uri.replace(0, 12, "morningcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "morningcoin.lnk";
}
...
{
    return GetAutostartDir() / "morningcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("morningcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  morningcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("morningcoin-Qt"));
